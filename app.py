...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    X = np.random.rand(N) # random dataset of N values 1-0
    ...
    slope_more_extreme = np.mean([abs(s) >= abs(slope) for s in slopes])  # calculate proportion of slopes more extreme than observed
    intercept_extreme = np.mean([abs(i) >= abs(intercept) for i in intercepts])  # calculate proportion of intercepts more extreme than observed
...
def index():
    if request.method == "POST":
        ...
        print("Mean of simulated slopes:", np.mean(slopes))

        # 
        # Clear previous session data related to simulations
        session.clear() 
        ...
        session["slope_extreme"] = slope_more_extreme
        ...
        # Debugging: Print session values after storing them
        print("Session values after data generation:")
        print(f"N: {session.get('N')}")
        print(f"mu: {session.get('mu')}")
        print(f"sigma2: {session.get('sigma2')}")
        print(f"beta0: {session.get('beta0')}")
        print(f"beta1: {session.get('beta1')}")
        print(f"S: {session.get('S')}")
        ...
        return render_template(
            ...
            plot1=plot1,
            plot2=plot2,
            slope_extreme=slope_more_extreme,
...
def hypothesis_test():
    ...
    print("Session slopes:", session.get("slopes"))
    print("Session intercepts:", session.get("intercepts"))
    print(f"N: {session.get('N')}")
    N = int(session.get("N"))
    S = int(session.get("S"))
    ...
    fun_message = "It's the thought that counts, but your p-value is too small :(" if p_value <= 0.0001 else None
...
def confidence_interval():
    ...
    t_value = t.ppf(1 - (1 - confidence_level / 100) / 2, df=S - 1)
    ...
    plt.scatter(estimates, np.zeros_like(estimates), color="gray", label="Simulated Estimates", alpha=0.5)
    plt.axhline(0, color="black", linewidth=0.5)
    plt.axvline(true_param, color="green", linestyle="--", label="True Parameter")
    plt.errorbar(mean_estimate, 0, xerr=[[mean_estimate - ci_lower], [ci_upper - mean_estimate]], color="blue", fmt="o", label=f"{confidence_level}% Confidence Interval")
